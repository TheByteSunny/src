...
#define PITCH_CURVE_LINEAR                                                                                                                                                                                                                     \
	45	// pitch greater than this is copied out                                                                                                                                                                                               \
		// spline in between
...
// Purpose:
//-----------------------------------------------------------------------------
class C_PropVehicleChair : public C_PhysicsProp, public IClientVehicle
{
	DECLARE_CLASS( C_PropVehicleChair, C_PhysicsProp );
...
IMPLEMENT_CLIENTCLASS_DT( C_PropVehicleChair, DT_PropVehicleChair, CPropVehicleChair )
RecvPropEHandle( RECVINFO( m_hPlayer ) ), RecvPropBool( RECVINFO( m_bEnterAnimOn ) ), RecvPropBool( RECVINFO( m_bExitAnimOn ) ), RecvPropVector( RECVINFO( m_vecEyeExitEndpoint ) ),
	END_RECV_TABLE()

		BEGIN_DATADESC( C_PropVehicleChair ) DEFINE_EMBEDDED( m_ViewSmoothingData ),
	END_DATADESC()

	//-----------------------------------------------------------------------------
	// Purpose:
	//-----------------------------------------------------------------------------
	C_PropVehicleChair::C_PropVehicleChair( void )
{
...
// Input  : updateType -
//-----------------------------------------------------------------------------
void C_PropVehicleChair::PreDataUpdate( DataUpdateType_t updateType )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropVehicleChair::PostDataUpdate( DataUpdateType_t updateType )
{
...
// Purpose:
//-----------------------------------------------------------------------------
C_BaseCombatCharacter *C_PropVehicleChair::GetPassenger( int nRole )
{
...
//-----------------------------------------------------------------------------
int C_PropVehicleChair::GetPassengerRole( C_BaseCombatCharacter *pPassenger )
{
...
//-----------------------------------------------------------------------------
void C_PropVehicleChair::GetVehicleViewPosition( int nRole, Vector *pAbsOrigin, QAngle *pAbsAngles, float *pFOV /*=NULL*/ )
{
...
//-----------------------------------------------------------------------------
void C_PropVehicleChair::GetVehicleClipPlanes( float &flZNear, float &flZFar ) const
{
...
//-----------------------------------------------------------------------------
void C_PropVehicleChair::DrawHudElements()
{
}
